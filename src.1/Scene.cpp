...
Scene::Scene() {
    ...
    // Get the root Node = obtenir le noeud racine 
    m_Root = m_Nodes.get("Root");

    // Create a projective default camera with standard parameter and place it in the scene = créer une caméra projective par défaut avec un parametres standard 
    // et la placer dans la scene
    ...
    // Create a Scene Node and link it to root node =  creer un noeud dans la scene et lier au noeud racine 
...
//obtenir le repere 3D du noeud racine 
Frame *Scene::frame() {
    return m_Root->frame();
}
